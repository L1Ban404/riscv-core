...
`u_pc_fifo` 是 `common_cells::stream_fifo`，配置为 `FALL_THROUGH=1`，默认深度
为 1。它记录已完成请求握手的 PC 和 epoch，并在对应响应被接受时弹出。
...
当请求先握手、响应延迟返回时，唯一的 FIFO entry 保存该请求。FIFO 满期间不再
产生下一笔请求。当前 `stream_fifo` 在满状态不支持同周期 pop/push，因此延迟
响应完成后，下一笔请求从后续周期开始，会在每次延迟响应后产生一个请求气泡。
...
- 接受一笔请求并延迟返回响应。
- 验证响应返回前不会展示或接受第二笔请求。
- 验证响应握手周期仍不进行满 FIFO 的同周期替换。
- 验证下一顺序 PC 从后续周期重新开始请求。
...
- `if_stage_tb.sv` 中 redirect reason 固定为 `REDIR_BRANCH`。当前 IF 逻辑只使用
  `valid` 和 `target_pc`。
- 默认 depth=1 时，延迟响应完成后存在一个请求气泡；寄存器 L0 同周期命中路径
  不受影响，仍可保持每周期一次取指握手。
